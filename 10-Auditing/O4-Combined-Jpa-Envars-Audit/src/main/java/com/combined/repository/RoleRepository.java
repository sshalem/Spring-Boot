package com.combined.repository;
...
@Repository
public interface RoleRepository extends JpaRepository<RoleEntity, Long>, RevisionRepository<RoleEntity, Long, Long> {

	/**
	 * I Must remove from the toString() methods in the Entities the
	 * associationEntity Otherwise the queries won't work Meaning If I have
	 * UserEntity with Set<RoleEntity> son't add the RoleEntity to the to String
	 * method We will get Stuck Overflow
	 */
	...
	/**
	 * @Query("SELECT r FROM RoleEntity r WHERE r.role = ?1") List<RoleEntity>
	 * jpqlFindByRole(String role);
	 */
	...
	/**
	 * we cannot set SELECT * * NonUniqueDiscoveredSqlAliasException: Encountered a
	 * duplicated sql alias [id] during auto-discovery of a native-sql query The
	 * NATIVE Query below does't work here in RoleRepo, This Native Query as is ,
	 * works great in UserRepo
	 */
