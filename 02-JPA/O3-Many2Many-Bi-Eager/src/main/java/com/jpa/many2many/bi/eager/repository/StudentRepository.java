...
public interface StudentRepository extends JpaRepository<StudentEntity, Long> {
	...
	@Query("SELECT ce FROM CourseEntity ce JOIN ce.students st WHERE ce.learningYear = :learningYear AND st.identityNumber = :identityNumber")
	List<CourseEntity> jpqlFindCoursesOfStudentInLearningYear(@Param("learningYear") int learningYear, @Param("identityNumber") int identityNumber);
	
	@Query(value = "SELECT * " + 
				   "FROM COURSE_TB ctb " +
				   "LEFT JOIN STUDENT_TB stb " +
				   "ON ctb.user_id=stb.id " +
				   "WHERE ctb.learningYear = :learningYear AND stb.identityNumber = :identityNumber", nativeQuery = true)
	List<CourseEntity> nativeFindCoursesOfStudentInLearningYear(@Param("learningYear") int learningYear, @Param("learningYear") int identityNumber);
	...
	@Query("SELECT cors FROM StudentEntity student JOIN student.courses cors WHERE student.identityNumber = :identityNumber")
	List<CourseEntity> jpqlFindAllCoursesOfStudentByIdentityNumber(@Param("identityNumber") int identityNumber);
	
	
	@Query("SELECT st FROM CourseEntity ce JOIN ce.students st WHERE ce.learningYear = :learningYear AND ce.courseNumber = :courseNumber")
	List<StudentEntity> jpqlFindStudentsWhoTookCourseInLearningYear(@Param("learningYear") int learningYear, @Param("courseNumber") String courseNumber);

	
	@Query("SELECT st FROM CourseEntity ce JOIN ce.students st WHERE ce.courseNumber = :courseNumber")
	List<StudentEntity> jpqlFindStudentsWhoTookCourseNumber(@Param("courseNumber") String courseNumber);
	
	

	
//	Position parameters not supported in Hibnerante  5.3
