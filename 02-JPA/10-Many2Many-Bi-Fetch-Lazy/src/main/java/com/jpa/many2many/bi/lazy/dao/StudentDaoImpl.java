...
import com.jpa.many2many.bi.lazy.exception.ResourceNotFoundException;
import com.jpa.many2many.bi.lazy.repository.CourseRepository;
...
//			throw new ResourceNotFoundException("Students with first name : " + firstName + " , Not Exist");
	...
	@Override
	public List<StudentDto> getStudentsWhoTookCourseInLearningYear(int learningYear, String courseNumber) {
		LOGGER.info("invoke getStudentsWhoTookCourseInLearningYear() ");
		List<StudentEntity> _students = studentRepository.jpqlFindStudentsWhoTookCourseInLearningYear(learningYear, courseNumber);
		
		List<StudentDto> _returnedValue = new ArrayList<>();
		
		
		for (StudentEntity student : _students) {
			StudentDto studentDto = new StudentDto();
			BeanUtils.copyProperties(student, studentDto);
			_returnedValue.add(studentDto);
		}
